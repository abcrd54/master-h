...
// embed.html is the public embed entry: it is designed to be framed by another
// app (the STB dashboard) on a sibling port, so it must not carry the SAMEORIGIN
// framing restriction that protects the SPA shell. Its own WebSocket/API surface
// stays protected by the Origin/Host allowlist plus the per-instance token.
const EMBED_HTML_PATH = '/embed.html';

function securityHeadersFor(urlPath: string): Record<string, string> {
    const headers: Record<string, string> = { 'X-Content-Type-Options': NOSNIFF };
    if (urlPath !== EMBED_HTML_PATH) {
...
export function createStaticHandler(publicDir: string): (req: IncomingMessage, res: ServerResponse) => void {
    return (req, res) => {
        ...
        const securityHeaders = securityHeadersFor(urlPath);
